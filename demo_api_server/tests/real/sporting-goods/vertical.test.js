...
describe(`Vertical manifest — ${VERTICAL} (real)`, () => {
  ...
  it('GET /api/config/vertical returns activeVertical=sporting-goods', async () => {
    const r = await client.get('/api/config/vertical');
    expect(r.status).toBe(200);
    expect(r.data.activeVertical).toBe(VERTICAL);
  });

  it('manifest terminology matches config/verticals/sporting-goods.json', async () => {
    const r = await client.get('/api/config/vertical');
  ...
  it('manifest identity matches config/verticals/sporting-goods.json', async () => {
    const r = await client.get('/api/config/vertical');
