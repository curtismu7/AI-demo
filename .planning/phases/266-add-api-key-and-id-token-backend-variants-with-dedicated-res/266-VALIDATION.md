...
| 266-01-02 | 01 | 1 | 266-R1, 266-R2, 266-R4 | T-CHAIN-AUTHOR | Gateway dispatch handler: api_key returns marker (no backend call); dual_token forwards bearer-only to `/api/resource-server/identity` (B1 fix: id_token NOT sent on the wire — read server-side from req.session.oauthTokens.idToken at the route); oauth_bearer forwards to `/accounts` or `/transactions`. Response includes `_meta.credentialPath` and `_meta.tokenEvents` (≥4 segments for dual_token, ≥2 for api_key, ≥1 for oauth_bearer). | unit | `cd banking_mcp_gateway && npx jest dispatch` | ❌ W0 | ⬜ pending |
...
| SQLite seed survives restart | 266-R2 | Persistence behavior across process restarts | Stop BFF, delete `banking_api_server/data/banking-resource-server.db`, restart BFF, confirm seed runs once (single startup log line); restart again, confirm seed does NOT re-run; query `/api/resource-server/accounts` after each restart confirms data persists. |
