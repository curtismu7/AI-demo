...
> Per-phase validation contract. Updated for R2 pivot: `banking_resource_server` = extended `routes/resourceServer.js`; new SQLite `banking.db`; Paths B and C both gated by existing `authenticateToken` middleware.
...
| 266-01-02 | 01 | 1 | 266-R1, 266-R2, 266-R4 | T-CHAIN-AUTHOR | Gateway dispatch handler: api_key returns marker (no backend call); dual_token forwards bearer + id_token to `/api/resource-server/identity` (id_token in JSON-RPC params, NOT header); oauth_bearer forwards to `/accounts` or `/transactions`. Response includes `_meta.credentialPath` and `_meta.tokenEvents` (≥4 segments for dual_token, ≥2 for api_key, ≥1 for oauth_bearer). | unit | `cd banking_mcp_gateway && npx jest dispatch` | ❌ W0 | ⬜ pending |
...
| 266-02-01 | 02 | 2 | 266-R2 | T-DB-INIT, T-RS-ACCOUNTS | `services/bankingDb.js`: `initBankingDb()` creates schema + seeds from `data/store.js` when banking.db missing; idempotent (second call leaves row count unchanged via `fs.existsSync` gate); parameterized queries (no string concat in WHERE clauses) | regression + integration | `cd banking_api_server && npx jest bankingDb.regression bankingDb.integration` | ❌ W0 | ⬜ pending |
| 266-02-02a | 02 | 2 | 266-R2 | T-RS-IDENTITY | `GET /api/resource-server/identity`: 401 on missing/invalid bearer (proves authenticateToken gating from `server.js:846`); 412 when session has no id_token; 200 returns `{ accessTokenClaims, idTokenClaims }` with NO JWT-shaped string in body (scrubRawJwts walker) | regression + integration | `cd banking_api_server && npx jest resourceServerIdentity.regression resourceServerIdentity.integration` | ❌ W0 | ⬜ pending |
| 266-02-02b | 02 | 2 | 266-R2 | T-RS-ACCOUNTS | `GET /api/resource-server/accounts`: 401 on missing/invalid bearer; 200 reads `getAccountsByUserId(req.user.sub)` from banking.db; existing `/summary` route still returns 200 (preserved untouched) | regression | `cd banking_api_server && npx jest resourceServerAccounts.regression` | ❌ W0 | ⬜ pending |
...
- [ ] `banking_api_server/routes/__tests__/resourceServerAccounts.regression.test.js` — 401 + read from banking.db
...
| SQLite seed survives restart | 266-R2 | Persistence behavior across process restarts | Stop BFF, delete `banking_api_server/data/banking.db`, restart BFF, confirm seed runs once (single startup log line); restart again, confirm seed does NOT re-run; query `/api/resource-server/accounts` after each restart confirms data persists. |
