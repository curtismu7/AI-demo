...
**Fix:** Move the four new specific rules above the broad `\brfc\b` rule at line 86.
They can sit right after the existing `langchain` block:

```js
// Insert these four before the broad \brfc\b guard (currently line 86):
...
**Fix:** Add a `useRef` for the dialog container and a `keydown` handler that
cycles focus among its focusable children:
...
<div
  ...
  onKeyDown={(e) => {
    if (e.key !== 'Tab') return;
    const focusable = discoveryDialogRef.current?.querySelectorAll(
...
`{isLoggedIn && renderActionGroups()}` duplicates the check.
...
### IN-03: `collapseAllGroups` / `expandAllGroups` iterate full `ACTION_GROUPS` regardless of `isConfigEmbeddedFocus` mode
...
write entries for all four groups into `chipGroupsState` and persist them to
localStorage. The button label can also read "Expand all" when only the single
admin group is displayed.
