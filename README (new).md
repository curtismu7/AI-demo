...
|---|---|---|
| **Agent (Digital Assistant)** | Agent Gateway | `aud: agent-gateway`, `sub: user`, `act: {sub: banking_agent}` |
| **Ping Authorize** | MCP | `aud: mcp`, `sub: user`, `act: {sub: banking_agent}` |
| **MCP** | Resource Server | `aud: resource-server`, `sub: user`, `act: {sub: banking_agent}` |
...
|---|---|---|
| **Ping Authorize** | Agent token | Retrieve claims to evaluate which tools the agent is permitted to see (`tools/list`); also fetches full tool list from MCP to apply fine-grained policy |
