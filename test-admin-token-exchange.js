...
/**
 * test-admin-token-exchange.js
 *
 * Comprehensive test suite for Phase 44 admin token exchange functionality.
...
// Test 2: Admin Token Requirement Detection
runTest('Admin Token Requirement Detection', () => {
  const adminReq = mockRequest(mockAdminSession.session);
  const userReq = mockRequest(mockUserSession.session);
  
  const shouldUseAdmin = adminTokenService.shouldUseAdminToken(adminReq);
  const shouldNotUseAdmin = adminTokenService.shouldUseAdminToken(userReq);
  
  if (shouldUseAdmin && !shouldNotUseAdmin) {
    return { passed: true, details: 'Correctly determined admin token usage requirements' };
  } else {
    return { passed: false, error: 'Admin token requirement detection failed', details: { shouldUseAdmin, shouldNotUseAdmin } };
  }
});
...
// Test 7: Admin Token Info Extraction
runTest('Admin Token Info Extraction', () => {
  const adminToken = adminTokenService.getAdminTokenFromSession(mockAdminSession.session);
  const tokenInfo = adminTokenService.getAdminTokenInfo(adminToken);
  
  if (tokenInfo && tokenInfo.clientId && tokenInfo.scopes && tokenInfo.expiresAt) {
    return { passed: true, details: 'Successfully extracted admin token information', details: { clientId: tokenInfo.clientId, scopes: tokenInfo.scopes } };
  } else {
    return { passed: false, error: 'Admin token info extraction failed', details: { tokenInfo } };
  }
});

// Test 8: Performance Test - Admin Token Detection
runTest('Performance Test - Admin Token Detection', () => {
  const iterations = 10000;
  const startTime = Date.now();
  
  for (let i = 0; i < iterations; i++) {
    adminTokenService.isAdminSession(mockAdminSession.session);
    adminTokenService.shouldUseAdminTokenForTool(mockRequest(mockAdminSession.session), 'admin_list_all_users');
  }
  ...
  // Performance should be under 1ms per operation
  if (avgTime < 1) {
    return { passed: true, details: `Performance test passed - ${avgTime.toFixed(3)}ms per operation`, details: { iterations, duration, avgTime } };
  } else {
    return { passed: false, error: `Performance test failed - ${avgTime.toFixed(3)}ms per operation (should be < 1ms)`, details: { iterations, duration, avgTime } };
  }
});

// Test 9: Security Test - Token Leakage Prevention
runTest('Security Test - Token Leakage Prevention', () => {
  const adminToken = adminTokenService.getAdminTokenFromSession(mockAdminSession.session);
  const tokenInfo = adminTokenService.getAdminTokenInfo(adminToken);
  
  // Check that sensitive token data is not exposed
  const hasAccessToken = tokenInfo.accessToken !== undefined;
  const hasRefreshToken = tokenInfo.refreshToken !== undefined;
  const hasIdToken = tokenInfo.idToken !== undefined;
  
  // Should not expose raw tokens, only metadata
  if (!hasAccessToken && !hasRefreshToken && !hasIdToken) {
    return { passed: true, details: 'Security test passed - no raw tokens exposed', details: { hasAccessToken, hasRefreshToken, hasIdToken } };
  } else {
    return { passed: false, error: 'Security test failed - raw tokens may be exposed', details: { hasAccessToken, hasRefreshToken, hasIdToken } };
  }
});

// Test 10: Configuration Test - Admin Token Config
runTest('Configuration Test - Admin Token Config', () => {
  const config = adminTokenService.ADMIN_TOKEN_CONFIG;
  
  const hasRequiredScopes = config.adminScopes && config.adminScopes.length > 0;
  const hasClientId = config.adminClientId !== undefined;
  const hasLifetime = config.adminTokenLifetime > 0;
  
  if (hasRequiredScopes && hasClientId && hasLifetime) {
    return { passed: true, details: 'Configuration test passed - admin token config is valid', details: { scopesCount: config.adminScopes.length, hasClientId, adminTokenLifetime: config.adminTokenLifetime } };
  } else {
    return { passed: false, error: 'Configuration test failed - admin token config is invalid', details: { hasRequiredScopes, hasClientId, hasLifetime } };
  }
});

// Run all tests
console.log('🚀 Starting Phase 44 Admin Token Exchange Tests\n');
...
function displayResults() {
  ...
  console.log('\n🎉 Phase 44 Admin Token Exchange Testing Complete!');
