...
- **Net:** the binding criterion is satisfied for the path the extraction actually changed (Condition 1 — live RFC 8693/gateway/MCP through `runMcpToolPipeline`, passed repeatedly, real PingOne login, `tokenEvents` asserted). Conditions 2-3 + the live 401 did not execute, but for a reason **wholly unrelated to this refactor and unrelated to Helix configuration**: the spec's own precondition probe phrase is no longer heuristic-proof in the current build. **Residual risk (reduced):** the live no-token-401 hard-fail and dead-Helix-fallback paths were not exercised live; they ARE covered by the static characterization suite's negative-path tests (`mcpToolPipeline.characterization.test.js` — the no-bearer/401 and conn-fallback Outcomes). **Follow-up (test-harness, NOT a config action — separate from this refactor):** update the Condition 2 precondition probe in `all-chips-pipeline.real.spec.js` to a phrase the current heuristic genuinely cannot resolve (or assert `source` over a known LLM-only chip), then re-run to green Conditions 2-3 + the 401. Do **not** "configure Helix" — it is already configured/vault-sourced.
