...
| **HITL PingOne MFA branch** | **`ff_hitl_pingone_mfa_enabled=true` + amount ≥ `confirm_stepup_threshold_usd` ($500) takes PingOne MFA path; flag off or amount below threshold takes homegrown email OTP — unchanged. `verifyAndConsumeChallenge()` is path-agnostic (untouched). `verifyOtp()` rejects MFA-path challenges (409 `not_mfa_path`). `submitOtp` uses worker token + `POST /deviceAuthentications/{daId}` (not `/otp` sub-path). Transfer HITL rule (§1 row above) unaffected — branching is on amount, not type.** | `demo_api_server/services/transactionConsentChallenge.js` — `confirmChallenge()` MFA branch, `verifyMfa()`, `selectMfaDevice()`, `getChallengePath()`. `demo_api_server/services/mfaService.js` — `initiateDeviceAuth` returns `{ id, _embedded.devices }` (not `daId`/`devices`); `submitOtp` uses worker token. `demo_api_server/routes/transactions.js` — `POST /consent-challenge/:id/confirm` branches on `mfaRequired`; `POST /verify-otp` detects `mfaPath` via `getChallengePath()`; new `POST /select-device` route. `demo_api_ui/src/components/TransactionConsentModal.tsx` — `handleConfirm` branches on `data.mfaRequired`; device picker state machine; `handleVerifyOtp` sends `{ deviceId, otp }` on MFA path. Tests: `transactionConsentChallenge.test.js` (16 tests), `hitlPingOneMfa.integration.test.js` (6 tests). |
