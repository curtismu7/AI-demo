...
- **Live chip gate — WAIVED (environment, not code):** Plan Task 6 made a live customer-login → "My Accounts" chip → `POST /api/mcp/tool` 200 + non-empty `tokenEvents` + Token Chain render (+ transfer step-up still gates) the *binding* exit criterion, precisely because §4 history shows the unit suite has missed regressions on this path. It could **not** be executed: this workspace has an encrypted `secrets.vault` but no `VAULT_PASSWORD` in `.env`/env/shell, so the BFF cannot start with PingOne config (boots with all `PINGONE_*` missing; `run-bank.sh` refuses). Owner accepted static verification (32/32 committed-state char+r1Local, dual spec+code-quality review per task, independent byte-equivalence diff) as closure. **Residual risk (explicit):** the one regression class the plan flagged as unit-test-invisible — a live RFC 8693 / PingOne-policy failure that only manifests against a real PingOne tenant — is NOT exercised by this verification. Before this branch ships, a maintainer with a working vault MUST run the live chip gate (steps in plan Task 6 Step 3-5) and append a confirming note here.
