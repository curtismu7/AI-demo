...
- **Live chip gate — PARTIALLY SATISFIED (core path passed live; LLM-routing conditions env-blocked):** Plan Task 6 made a live customer-login → chip → `POST /api/mcp/tool` 200 + non-empty `tokenEvents` the *binding* exit criterion, precisely because §4 history shows the unit suite has missed regressions on this path. It was **initially waived** (workspace had no `VAULT_PASSWORD`); the environment was subsequently fixed (`.env` gained `VAULT_PASSWORD` + real `PINGONE_*`), the stack was restarted on this branch's thin-shell code (BFF PID verified changed; `runMcpToolPipeline` confirmed in the running `server.js`), and the skip-proof `banking_api_ui/tests/e2e/all-chips-pipeline.real.spec.js` was run against `https://api.ping.demo:4000` with the demo customer+admin creds (2026-05-18). **Result:**
  ...
  - ❌ Condition 2 — Helix-only routing: failed fast (272ms/207ms) on its **own explicit hard-gate precondition** — `"Helix is NOT configured (probe did not return source=helix)... Helix creds are NOT vault-sourced"` (BFF banner confirms `[HELIX LLM] not configured`). The spec aborts before exercising the pipeline; the one downstream `got 502` (`spending_trends`) is the unconfigured-Helix route failing to resolve, **not** a token-exchange/gateway/MCP defect in the extracted code.
  - ⚪ Condition 3 (dead-Helix fallback) + the no-token 401 hard-fail: **did not run** (suite halted after Cond 2 exhausted retries).
- **Net:** the binding criterion is satisfied for the path the extraction actually changed (Condition 1 — live RFC 8693/gateway/MCP through `runMcpToolPipeline`, passed repeatedly). The unexecuted/failed conditions are blocked by Helix being unconfigured in this workspace (a known, non-vault-sourced environment limitation unrelated to this refactor), not by the pipeline. **Residual risk (reduced but not zero):** the live no-token-401 hard-fail and dead-Helix-fallback paths were not exercised live (they ARE covered by the static characterization suite's negative-path tests). Before ship, a maintainer with Helix configured (`/setup`, `HELIX_API_KEY`, or `LLM2.json`) SHOULD re-run the full spec to green Conditions 2-3 + the 401 hard-fail and append a confirming note here.
