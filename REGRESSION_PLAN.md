...
- `banking_api_ui/src/config/diagram-token-flow-regions.js` — added a `hitl-tf` highlight region; header note records that the regenerated PNG shifted the sequence-column layout so the cosmetic overlay bounds are approximate (no logic impact).
