...
| **MCP Authorize gate (runs every tool call)** | **`ff_authorize_mcp_first_tool = true` blocks `POST /api/mcp/tool` on every call — not just the first. Gate evaluates aud/scope and amount-based business rules (HITL for transfers ≥ confirm threshold, step-up for amounts ≥ step-up threshold). `req.session.mcpFirstToolAuthorizeDone` is no longer used as a skip flag.** With PingOne unavailable and `ff_authorize_fail_open = false`, gate returns 503. Write tools (`create_transfer`, `create_deposit`, `create_withdrawal`) pass `toolParams.amount` + `transactionType` into the evaluator. | `banking_api_server/services/mcpToolAuthorizationService.js` — `evaluateMcpFirstToolGate()` (accepts `toolParams`); `banking_api_server/services/simulatedAuthorizeService.js` — `evaluateMcpFirstTool()` (amount-based HITL/step-up); `banking_api_server/server.js` — gate block in `POST /api/mcp/tool` passes `toolParams: params`; `banking_api_server/routes/featureFlags.js` — `ff_authorize_mcp_first_tool`. Status at `GET /api/authorize/evaluation-status` (admin). |
