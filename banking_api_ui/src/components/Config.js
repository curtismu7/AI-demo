    ...
    <div className={`card config-page__section ${className}`}>
      ...
      >
        <div className="config-page__section-toggle-left">
          ...
          {subtitle && (
            <span className="config-page__section-subtitle">{subtitle}</span>
          )}
        </div>
      ...
      {open && (
        <div
          className={`config-page__section-body${passthrough ? " config-page__section-body--passthrough" : ""}`}
        >
          {children}
        </div>
      )}
    ...
    <div className="form-group">
      ...
      <div style={{ position: "relative", display: "flex", gap: "0.5rem" }}>
        <input
          ...
          placeholder={
            isSet ? "Leave blank to keep current value" : "Enter value…"
          }
          autoComplete="off"
          style={{ flex: 1 }}
          disabled={disabled}
...
function DisplayPreferences() {
  const [mode, setMode] = useState(
    () => localStorage.getItem(DISPLAY_MODE_KEY) || "panel",
  );
  ...
  return (
    ...
    >
      ...
      >
        Controls where banking results (accounts, transactions, balances) appear
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: "0.78rem", color: "#374151" }}>
              Results appear in the Agent chat panel alongside your
              conversation. Good for quick lookups without leaving the current
              view.
            </div>
          </div>
        </label>
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: "0.78rem", color: "#374151" }}>
              Results update the main dashboard in the background — same account
              cards and transaction tables as the full page, just triggered by
              the Agent.
            </div>
          </div>
        </label>
      ...
      {mode === "fullpage" && (
        ...
        >
          💡 Full Page mode: Agent results will update the account cards and
          transaction table on your dashboard. The Agent chat stays open for
          follow-up questions.
        </div>
      )}
    </CollapsibleCard>
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: "0.78rem", color: "#374151" }}>
              Agent immediately returns a job ID and polls visibly: &quot;Job
              created: #abc123, checking status…&quot; — shows the async pattern
              explicitly.
            </div>
          </div>
        </label>
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: "0.78rem", color: "#374151" }}>
              Agent polls silently in the background and shows only the final
              result. Simulates a &quot;fire and forget&quot; experience without
              visible async state.
            </div>
          </div>
        </label>
      ...
      {mode === "job-id" && (
        ...
        >
          💡 Job ID mode: the agent will report &quot;Job #abc123 created&quot;
          and poll until complete — great for demos showing async patterns.
        </div>
      )}
    </CollapsibleCard>
  );
...
function LangChainAgentConfig() {
  ...
  const handleClearKey = async (keyType) => {
    try {
      const r = await fetch("/api/langchain/config/key/" + keyType, {
        method: "DELETE",
      });
      const d = await r.json();
      if (d.ok) {
        setStatus((prev) => ({
          ...prev,
          key_set: { ...prev.key_set, [keyType]: false },
        }));
  ...
  if (!status)
    return (
      <p style={{ padding: "8px", color: "#374151" }}>
        Loading LangChain config…
      </p>
    );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {activeProvider !== "ollama" ? (
        ...
        >
          <span style={{ fontSize: 13, fontWeight: 500, minWidth: 90 }}>
            API Key:
          </span>
          {status.key_set?.[activeProvider] ? (
            ...
            <>
              <input
                type="password"
                placeholder={
                  PROVIDERS.find((p) => p.id === activeProvider)?.placeholder ||
                  ""
                }
          ...
          {messages[activeProvider] && (
            <span
              style={{
                fontSize: 12,
                color: messages[activeProvider].startsWith("✓")
                  ? "#2e7d32"
                  : "#c62828",
              }}
            >
        ...
        >
          Runs locally via LM Studio or Ollama — no API key needed. Make sure
          your local inference server is running.
        </p>
      )}

      <p style={{ margin: "4px 0 0", fontSize: 12, color: "#374151" }}>
        Keys are stored in your server session only and are never included in
        API responses. Refresh or logout to clear all keys.
      </p>
    </div>
  );
}
...
export default function Config() {
  ...
  const loadConfig = useCallback(async () => {
    ...
    try {
      ...
      setDeploymentManaged(
        !!data.deploymentManagedPingOneOAuth ||
          process.env.REACT_APP_MANAGED_DEPLOYMENT === "true",
      );
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...prev,
      admin_redirect_uri:
        prev.admin_redirect_uri || `${origin}/api/auth/oauth/callback`,
      user_redirect_uri:
        prev.user_redirect_uri || `${origin}/api/auth/oauth/user/callback`,
    ...
    <div className="config-page app-page-shell">
      <header className="app-page-shell__hero config-page__hero-shell">
        <div className="app-page-shell__hero-top">
          <div>
            <h1 className="app-page-shell__title">
              ⚙️ Application Configuration
            </h1>
            <p className="app-page-shell__lead">
              {deploymentManaged ? (
                <>
                  <strong>Hosted:</strong> <strong>Two</strong> PingOne OAuth
                  apps (admin + end-user) — client credentials live{" "}
                  <strong>on the server</strong> (secrets / KV — not entered by
                  visitors). Use <strong>Admin</strong> and{" "}
                  <strong>Customer</strong> sign-in on the login page. Register
                  both redirect URIs below in PingOne.{" "}
                </>
              ) : (
                <>
                  <strong>Local:</strong> configure <strong>both</strong>{" "}
                  PingOne apps here (admin + end-user). Stored in SQLite.{" "}
                </>
              )}
            </p>
          </div>
          <div className="app-page-shell__actions config-page__hero-actions">
            {isConfigured && (
              <span className="config-page__badge config-page__badge--ok">
                ✓ Configured
              </span>
            )}
            {!isConfigured && (
              <span className="config-page__badge config-page__badge--warn">
                ⚠ Not configured
              </span>
            )}
            <Link
              to="/onboarding"
              className="app-page-shell__btn app-page-shell__btn--solid"
            >
              Setup guide
            </Link>
            <Link to="/setup/pingone" className="app-page-shell__btn">
              PingOne reference
            </Link>
            <Link to="/" className="app-page-shell__btn">
              ← Back to app
            </Link>
            <button
              type="button"
      ...
      <div className="app-page-shell__body">
        <div className="config-page__main">
          ...
          {activeTab === "pingone-config" && <SetupWizardTab />}

          {/* Worker App tab content */}
          {activeTab === "worker" && <WorkerAppConfigTab />}
            ...
            <div className="card">
              ...
              <div className="card-body">
                <p style={{ marginBottom: "1rem", color: "#374151" }}>
                  Manage OAuth scopes in your PingOne environment. Create,
                  update, or remove scopes and grant them to applications.
                </p>
                <p style={{ marginBottom: "1.5rem", color: "#374151" }}>
                  <strong>Scope Update Tool:</strong> Go to the{" "}
                  <Link
                    to="/admin"
                    style={{
                      color: "var(--brand-navy)",
                  ...
                  → Banking Admin page to use the automated{" "}
                  <strong>Fix PingOne Scopes</strong> button.
                </p>
                <div
                  style={{
                ...
                >
                  ...
                  >
                    💡 <strong>Tip:</strong> Use the automated scope update tool
                    to:
                    <ul
                      style={{
                        marginTop: "0.5rem",
            ...
            <React.Fragment>
              ...
              {readOnly && (
                <div className="config-page__banner config-page__banner--info">
                  <strong>Read-only mode (hosted, no KV):</strong> Runtime
                  PingOne fields are supplied by the deployment (server-side).
                  Connect <strong>Upstash Redis / KV</strong> (
                  <code>KV_REST_API_URL</code>) if you need to edit values from
                  this UI. On <strong>localhost</strong> or{" "}
                  <strong>Replit with SQLite</strong>, configuration is stored
                  on disk.
                </div>
              )}
              ...
              {deploymentManaged && (
                <div className="config-page__banner config-page__banner--hosted">
                  ...
                  <strong>Client IDs and secrets</strong> are configured on the
                  server (not on this screen). You only need to{" "}
                  <strong>register the redirect URIs</strong> in PingOne for
                  your Admin and Customer apps — see the blue box below for the
                  exact values.
                </div>
              )}
              ...
              {demoMode && (
                <div className="config-page__banner config-page__banner--warning">
                  <strong>Demo mode:</strong> This is a shared public demo. All
                  banking data is simulated — no real transactions occur.
                  Transfers and account operations are limited. To use your own
                  data,{" "}
                  <button
                    style={{
                      background: "none",
              ...
              {redirectInfo && !redirectInfo.error && (
                ...
                >
                  <p className="config-page__redirect-lede">
                    {redirectInfo.instructions?.summary ||
                      "Each PingOne OAuth app must allowlist its callback URL exactly (scheme, host, path)."}{" "}
                    {redirectInfo.stableDemoOrigin && (
                      <span>
                        Production alias for this demo:{" "}
                        <code
                          style={{
                  ...
                  <div style={{ display: "grid", gap: "1rem" }}>
                    <div>
                      <div className="config-page__redirect-label">
                        Admin (staff) app — Redirect URI
                      </div>
                      <div
                        style={{
                          display: "flex",
                      ...
                      >
                        <code className="config-page__code-block">
                          {redirectInfo.adminRedirectUri}
                        </code>
                        <button
                          type="button"
                          className="btn btn-secondary"
                          ...
                          onClick={() => {
                            ...
                            notifySuccess("Admin redirect URI copied");
                          }}
                        >
                          Copy
                        </button>
                    ...
                    <div>
                      <div className="config-page__redirect-label">
                        Customer (end-user) app — Redirect URI
                      </div>
                      <div
                        style={{
                          display: "flex",
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            notifySuccess("Customer redirect URI copied");
                          }}
                        >
                          Copy
                        </button>
                  ...
                  {Array.isArray(redirectInfo.referenceRedirectSets) &&
                    redirectInfo.referenceRedirectSets.length > 0 && (
                      <div
                        style={{
                          marginTop: "1rem",
                      ...
                      >
                        ...
                        >
                          Reference: localhost + api.pingdeme.org (add every
                          host you use in PingOne)
                        </p>
                        {redirectInfo.referenceRedirectSets.map((row) => (
                          <div key={row.id} style={{ marginBottom: "0.85rem" }}>
                            ...
                            >
                              ...
                              >
                                <span
                                  style={{ minWidth: "7rem", color: "#374151" }}
                                >
                                  Admin
                                </span>
                                <code
                                  className="config-page__code-block"
                                  style={{ flex: "1 1 200px" }}
                                >
                                  {row.adminRedirectUri}
                                </code>
                                <button
                                  type="button"
                                  className="btn btn-secondary"
                  ...
                  {redirectInfo.canonicalOrigin && (
                    <p className="config-page__redirect-foot">
                      Canonical origin:{" "}
                      <code>{redirectInfo.canonicalOrigin}</code>
                      {redirectInfo.requestHost && (
                        <>
                          {" "}
                          · Request host:{" "}
                          <code>{redirectInfo.requestHost}</code>
                        </>
                      )}
                    </p>
                  )}
              ...
              {redirectInfo?.error && (
                <div
                  className="card config-page__banner--danger"
                  style={{ marginBottom: "1.5rem" }}
                >
                  <p style={{ margin: 0, fontSize: "0.875rem" }}>
                    Could not load redirect URI info: {redirectInfo.error}
                  </p>
                </div>
              )}
              ...
              {!deploymentManaged && (
                <div className="config-page__banner config-page__banner--success">
                  <strong>Local development:</strong> Configure{" "}
                  <strong>Admin</strong> and <strong>End-User</strong> OAuth
                  apps independently —{" "}
                  <strong>two PingOne apps and two client IDs</strong>, same
                  model as production. This layout is shown when
                  deployment-managed OAuth is off (e.g. local or Replit with
                  full editor).
                </div>
              )}
              ...
              >
                <p className="config-page__steps-intro">
                  For a printable-style checklist, open the{" "}
                  <Link to="/onboarding">onboarding guide</Link>. Follow the
                ...
                <ol className="config-page__steps-list">
                  {deploymentManaged ? (
                    <>
                      <li>
                        Copy the <strong>Admin</strong> and{" "}
                        <strong>Customer</strong> redirect URIs from the{" "}
                        <strong>Register these redirect URIs in PingOne</strong>{" "}
                        section above into each PingOne application's allowlist.
                      </li>
                      <li>
                        OAuth client IDs and secrets are stored{" "}
                        <strong>on the server</strong> — visitors do not enter
                        them here.
                      </li>
                      <li>
                        Use <strong>Admin sign-in</strong> and{" "}
                        <strong>Customer sign-in</strong> on the login page.
                      </li>
                      <li>
                        Optional: other fields below may be read-only unless KV
                        is connected.
                      </li>
                    </>
                  ) : (
                    <>
                      <li>
                        In PingOne, create <strong>two</strong> OIDC web
                        applications (admin + end user). Copy each{" "}
                        <strong>redirect URI</strong> into the matching PingOne
                        app — exact match required.
                      </li>
                      <li>
                        Enter <strong>Environment ID</strong>,{" "}
                        <strong>region</strong>, and{" "}
                        <strong>frontend URL</strong>, then{" "}
                        <strong>Test PingOne Connection</strong>.
                      </li>
                      <li>
                        Enter each app&apos;s <strong>client ID</strong> and{" "}
                        <strong>client secret</strong>. Leave secrets blank if
                        &quot;already set&quot; and you are not rotating them.
                      </li>
                    </>
                  )}
                  <li>
                    Set a random <strong>session secret</strong> (32+
                    characters) and the{" "}
                    <strong>admin / customer role names</strong> that match your
                    PingOne setup.
                  </li>
                  <li>
                    Optional: <strong>PingOne Authorize</strong> for transfer
                    policy; <strong>Advanced</strong> / MCP URL is for local
                    use.
                  </li>
                  <li>
                    <strong>Save Configuration</strong>, then sign in with Admin
              ...
              {!isConfigured && !readOnly && !deploymentManaged && (
                <div className="config-page__banner config-page__banner--warning">
                  <strong>Not saved yet:</strong> Complete the fields below,
                  then <strong>Save Configuration</strong>. Values are stored in{" "}
                  {storageType === "vercel-kv" ? "Redis (KV)" : "SQLite"} so
              ...
              <form id="config-main-form" onSubmit={handleSave}>
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.checked) {
                                ...
                                if (eff.size === 0) {
                                  notifyError(
                                    "Select at least one Agent MCP scope.",
                                  );
                                  return;
                                }
                              }
                          ...
                          <span className="config-page__agent-scope-body">
                            <span className="config-page__agent-scope-label">
                              {row.label}
                            </span>
                            <code className="config-page__agent-scope-code">
                              {row.scope}
                            </code>
                            <span className="config-page__agent-scope-desc">
                              {row.description}
                            </span>
                          </span>
                        </label>
                ...
                >
                  <div className="config-page__grid">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleChange("pingone_region", e.target.value)
                        }
                        disabled={readOnly || deploymentManaged}
                      >
                  ...
                  <div className="config-page__test-row">
                    <button
                      ...
                      disabled={
                        testing ||
                        !form.pingone_environment_id ||
                        !form.admin_client_id
                      }
                    >
                    ...
                    {testResult && (
                      <div
                        className={`config-page__test-result ${testResult.ok ? "config-page__test-result--ok" : "config-page__test-result--err"}`}
                      >
                        {testResult.ok
                          ? `✓ ${testResult.message}`
                          : `✗ ${testResult.message}`}
                        {testResult.ok && testResult.issuer && (
                          <div className="config-page__test-issuer">
                ...
                {deploymentManaged ? (
                  ...
                  >
                    <p
                      className="config-page__card-subtitle"
                      style={{ margin: 0 }}
                    >
                      Admin and Customer apps use client credentials stored{" "}
                      <strong>on the backend</strong>. Redirect URIs you must
                      add in PingOne are in the{" "}
                      <strong>"Register these redirect URIs"</strong> section
                      above (copy/paste). You do not type client secrets on this
                      page. Optional <strong>pi.flow</strong> authorize: set
                      deployment env{" "}
                      <code>PINGONE_ADMIN_AUTHORIZE_PI_FLOW=true</code> or{" "}
                      <code>PINGONE_USER_AUTHORIZE_PI_FLOW=true</code> only when
                      your PingOne apps support it (
                      <a
                        href="https://developer.pingidentity.com/pingone-api/auth/auth-config-options/browserless-authentication-flow-options.html"
                        target="_blank"
                  ...
                  <>
                      ...
                      <div className="config-page__grid">
                        ...
                        <div className="form-group config-page__grid-span-2">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              handleChange(
                                "admin_pingone_authorize_pi_flow",
                                e.target.value,
                              )
                            }
                            disabled={readOnly}
                          >
                            <option value="false">
                              Standard (response_type=code, redirect with
                              ?code=)
                            </option>
                            <option value="true">
                              pi.flow — response_type=pi.flow +
                          ...
                          >
                            Enable only when this PingOne app supports
                            non-redirect / DaVinci flow authorize (see{" "}
                            <a
                              href="https://developer.pingidentity.com/pingone-api/auth/auth-config-options/browserless-authentication-flow-options.html"
                              target="_blank"
                              rel="noopener noreferrer"
                            >
                              PingOne: redirect and non-redirect flows
                            </a>
                    ...
                    >
                      <div className="config-page__grid">
                        ...
                        <div className="form-group config-page__grid-span-2">
                          <label className="form-label">
                            PingOne authorize — pi.flow (end-user)
                          </label>
                          <select
                            className="form-input"
                            ...
                            onChange={(e) =>
                              handleChange(
                                "user_pingone_authorize_pi_flow",
                                e.target.value,
                              )
                            }
                            disabled={readOnly}
                          ...
                          >
                            Applies to <strong>all</strong> customer logins when
                            enabled. For marketing-only pi.flow, use{" "}
                            <strong>Marketing customer sign-in</strong> below
                            instead.
                          </p>
                        </div>
                      </div>
                    ...
                    >
                      <div className="config-page__grid">
                        <div className="form-group config-page__grid-span-2">
                          <label className="form-label">
                            Customer login on marketing page
                          </label>
                          <select
                            className="form-input"
                          ...
                          >
                            <option value="redirect">
                              Redirect — standard authorize (code + PKCE)
                            </option>
                            <option value="slide_pi_flow">
                              Slide panel — hints + Continue with pi.flow
                              (?use_pi_flow=1)
                            </option>
                          </select>
                          <p
                          ...
                          >
                            <strong>Redirect</strong> is the safe default.{" "}
                            <strong>pi.flow</strong> requires a PingOne OIDC app
                            that supports it — otherwise use{" "}
                            <strong>Redirect</strong> (same choice as Demo
                            config).
                          </p>
                        </div>
                        <div className="config-page__grid-span-2">
                  ...
                  <div className="config-page__grid">
                    <div className="form-group">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleChange("step_up_method", e.target.value)
                        }
                        disabled={readOnly}
                      >
                        <option value="ciba">
                          CIBA — back-channel challenge (inline, no page
                          redirect)
                        </option>
                        <option value="email">
                          Email / OTP — OIDC re-authentication redirect
                        </option>
                      </select>
                      <p
                        style={{
                      ...
                      >
                        <strong>CIBA:</strong> the dashboard shows a "Verify via
                        CIBA" button that sends a back-channel auth request; the
                        user approves on their registered device.{" "}
                        <strong>Email / OTP:</strong> the browser redirects to
                        PingOne for a fresh MFA sign-in (email code, TOTP,
                        etc.).
                      </p>
                    </div>
                    <div className="form-group">
                      ...
                      >
                        Minimum transaction amount (USD) that triggers step-up
                        authentication. Default: $250.
                      </p>
                    </div>
                    <div className="form-group">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleChange("ciba_enabled", e.target.value)
                        }
                        disabled={readOnly}
                      >
                      ...
                      >
                        Required when step-up method is CIBA. Also controls the
                        CIBA panel on the demo pages.
                      </p>
                    </div>
                  </div>
                </CollapsibleCard>
                ...
                >
                  <div className="config-page__grid">
                    <div className="form-group">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleChange("authorize_enabled", e.target.value)
                        }
                        disabled={readOnly}
                      >
                        <option value="false">Disabled</option>
                        <option value="true">
                          Enabled — enforce on transfers &amp; withdrawals
                        </option>
                      </select>
                      <p
                        style={{
                      ...
                      >
                        When enabled, every transfer and withdrawal is evaluated
                  ...
                  {deploymentManaged ? (
                    <div
                      className="config-page__note-box"
                      style={{ marginTop: "1rem" }}
                    >
                      <strong>Worker app credentials</strong> for PingOne
                      Authorize are configured in the deployment (same as OAuth
                      clients). They are not edited on this page.
                    </div>
                  ) : (
                    <div
                      className="config-page__grid"
                      style={{ marginTop: "1rem" }}
                    >
                      <TextField
                ...
                >
                  ...
                  {storageType === "vercel-kv" && (
                    <div className="config-page__callout">
                      ...
                      <div>
                        <strong>
                          LangChain / MCP Agent not available on hosted cloud.
                        </strong>{" "}
                        The agent runs as a local Python process and cannot be
                        reached from a browser-only cloud deployment (Vercel,
                        Replit web, etc.). Leave this field blank — the chat
                        panel will show a "not configured" message instead of
                        failing silently.
                      </div>
                    </div>
                  )}
                  ...
                  <div className="config-page__grid">
                    <TextField
                      ...
                      placeholder={
                        storageType === "vercel-kv"
                          ? "Not available on hosted cloud — leave blank"
                          : "http://localhost:8000"
                      }
                      help={
                        storageType === "vercel-kv"
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleChange("debug_oauth", e.target.value)
                        }
                        disabled={readOnly}
                      >
                  ...
                  {storageType === "vercel-kv" && !readOnly && (
                    <div className="config-page__divider">
                      <div
                        className="config-page__callout config-page__callout--purple"
                        style={{ marginBottom: "0.75rem" }}
                      >
                        <strong>🔑 Config password</strong> — once credentials
                        are saved, updates require an admin password. Set{" "}
                        <code>ADMIN_CONFIG_PASSWORD</code> in your Replit /
                        Vercel secrets variables, then enter it here before
                        saving. Leave blank on first-time setup.
                      </div>
                      <div className="form-group" style={{ maxWidth: "400px" }}>
                        <label className="form-label">
                          Config Password (hosted KV)
                        </label>
                        <input
                          type="password"
                          className="form-input"
                        ...
                        >
                          Required to overwrite config when the API uses this
                          gate. Not stored — enter each session.
                        </p>
                      </div>
                    </div>
                  )}
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ fontSize: "0.8rem", color: "#92400e" }}>
                      Values the server is currently using — read from Vercel
                      environment variables. Compare these against your PingOne
                      app settings to diagnose login failures.
                    </span>
                  </div>
                  <div
                    style={{
                  ...
                  >
                    {[
                      {
                        label: "Environment ID",
                        value: redirectInfo.environmentId,
                      },
                      {
                        label: "Admin Client ID",
                        value: redirectInfo.adminClientId,
                      },
                      {
                        label: "Admin Secret",
                        value: redirectInfo.adminSecretSet
                          ? `${redirectInfo.adminSecretHint}•••• (set)`
                          : null,
                      },
                      {
                        label: "User Client ID",
                        value: redirectInfo.userClientId,
                      },
                      {
                        label: "User Secret",
                        value: redirectInfo.userSecretSet
                          ? `${redirectInfo.userSecretHint}•••• (set)`
                          : null,
                      },
                      {
                        label: "Admin Redirect URI",
                        value: redirectInfo.adminRedirectUri,
                      },
                      {
                        label: "User Redirect URI",
                        value: redirectInfo.userRedirectUri,
                      },
                    ].map(({ label, value }) => (
                      <div
                        key={label}
              ...
              <div className="config-page__expand-wrap">
                ...
                >
                  🚀 Run Your Own Instance
                  <span className="config-page__expand-chevron">
                    {showSelfHosting ? "▲" : "▼"}
                  </span>
                </button>
                ...
                {showSelfHosting && (
                  <div className="card config-page__self-host-card">
                    <div className="card-header">
                      ...
                      >
                        The hosted demo at{" "}
                        <code>banking-demo-puce.vercel.app</code> may use a
                        shared PingOne environment (see project docs for the
                        current URL). Your own deployment gets its own isolated
                        PingOne environment — users you create there are
                        separate.
                      </p>
                    </div>

                    <div
                      className="config-page__callout"
                      style={{ marginBottom: "1.5rem" }}
                    >
                      <strong>Client secrets are optional</strong> — both
                      deployment modes support PKCE (public client). Only expose
                      a client secret if your PingOne app is configured as
                      confidential.
                    </div>
                    ...
                    <div className="config-page__self-host-grid">
                      ...
                      <div className="config-page__path-card config-page__path-card--indigo">
                        <h3 className="config-page__path-title">
                          ☁️ Path A: Deploy (Vercel or Replit)
                        </h3>
                        <p className="config-page__path-desc">
                          Your own PingOne environment, zero-downtime deploys,
                          free tier available.
                        </p>
                        <ol className="config-page__path-steps">
                          ...
                          <li>
                            In PingOne, create{" "}
                            <strong>two OIDC web apps</strong> (Admin +
                            Customer) and note each Client ID
                          </li>
                          <li>
                            Set these environment variables on your host
                            (Vercel, Replit Secrets, etc.):
                            <table className="config-page__env-table">
                              <tbody>
                                {[
                                  [
                                    "PINGONE_ENVIRONMENT_ID",
                                    "Your PingOne env UUID",
                                  ],
                                  [
                                    "PINGONE_REGION",
                                    "com / eu / ca / asia / com.au",
                                  ],
                                  [
                                  ...
                                  [
                                    ...
                                    "Admin secret (or omit for PKCE-only)",
                                  ],
                                  [
                                    "PINGONE_USER_CLIENT_ID",
                                    "Customer app Client ID",
                                  ],
                                  [
                                    "PINGONE_USER_CLIENT_SECRET",
                                    "Customer secret (or omit)",
                                  ],
                                  [
                                    "PINGONE_ADMIN_ROLE",
                                  ...
                                  [
                                    "PINGONE_USER_ROLE",
                                    "Role name (default: customer)",
                                  ],
                                  ["SESSION_SECRET", "Random 32+ char string"],
                                  [
                                    "PUBLIC_APP_URL",
                                    "Your public URL (e.g. https://my-app.vercel.app or *.replit.dev)",
                                  ],
                                ].map(([k, v]) => (
                                  <tr key={k}>
                                    <td className="config-page__env-key">
                                      {k}
                                    </td>
                                    <td className="config-page__env-val">
                                      {v}
                                    </td>
                                  </tr>
                                ))}
                          ...
                          <li>
                            Add the redirect URIs shown on this page to each
                            PingOne app
                          </li>
                          <li>
                            <strong>
                              Replit-specific (optional but recommended):
                            </strong>{" "}
                            set <code>REPLIT_MANAGED_OAUTH=true</code> when this
                            page should be reference-only for OAuth credentials,
                            and set{" "}
                            <code>REPLIT_CONFIG_PASSWORD_MODE=true</code> +{" "}
                            <code>ADMIN_CONFIG_PASSWORD</code> if you want
                            password-gated config updates on hosted deployments.
                          </li>
                          <li>
                            <strong>Replit networking:</strong> if UI and API
                            are on different origins, set{" "}
                            <code>CORS_ORIGIN</code> to the UI URL. Keep{" "}
                            <code>PUBLIC_APP_URL</code> as your stable published
                            Replit HTTPS URL (no trailing slash).
                          </li>
                      ...
                      <div className="config-page__path-card config-page__path-card--green">
                        ...
                        <ol className="config-page__path-steps">
                          ...
                          <li>
                            Copy <code>banking_api_server/.env.example</code> →{" "}
                            <code>banking_api_server/.env</code> and fill in
                            your PingOne values
                          </li>
                          <li>
                          ...
                          <li>
                            <code>npm install &amp;&amp; npm run dev</code> in{" "}
                            <code>banking_api_server/</code>
                          </li>
                          <li>
                            <code>npm install &amp;&amp; npm start</code> in{" "}
                            <code>banking_api_ui/</code>
                          </li>
                          <li>
                            Open this Config page and follow the on-screen setup
                          </li>
                        </ol>
                      </div>
                    </div>
