...
export default function RunServersModal({ onClose }) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      // Terminal window will open and run-bank.sh will open the browser when ready
      onClose();
    } catch (_) {
      setStatus('error');
    }
        ...
        <div className="rsm-body">
          {status === 'confirm' && (
            <>
              ...
              <p className="rsm-note">A Terminal window will open. The browser will open automatically when the servers are ready.</p>
